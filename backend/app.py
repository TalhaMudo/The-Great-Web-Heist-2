...
class SearchResultDetail(BaseModel):
    """Extended result that includes the assignment-required triple plus
    optional metadata helpful for the dashboard."""
...
class SearchResponse(BaseModel):
    """Response intentionally exposes both shapes:

    - ``triples`` matches the assignment contract exactly:
      a list of ``(relevant_url, origin_url, depth)``.
    - ``results`` includes the same data with score/title for UI rendering.
    """

    triples: List[SearchTriple]
...
def _build_search_response(raw_results) -> SearchResponse:
    triples: List[SearchTriple] = []
    detailed: List[SearchResultDetail] = []
    for url, origin_url, depth, score, title in raw_results:
        triples.append(SearchTriple(relevant_url=url, origin_url=origin_url, depth=depth))
        detailed.append(
            SearchResultDetail(
                relevant_url=url,
    ...
    return SearchResponse(triples=triples, results=detailed)
...
async def search(query: str, limit: int | None = None) -> SearchResponse:
    ...
    return _build_search_response(raw_results)
