...
class LinkExtractor(html.parser.HTMLParser):
    """Pull href values out of <a> tags using only the standard library."""
...
async def fetch_html(url: str, timeout: float = 10.0) -> str | None:
    ...
    def _blocking_fetch() -> str | None:
        req = Request(url, headers={"User-Agent": "HeistCrawler/2.0 (multi-agent build)"})
...
class CrawlerService:
    """Async crawler that owns a fixed worker pool and a global queue cap.

    Back pressure is implemented by sharing a global queue capacity across all
    crawl jobs: when the cap is reached, link discovery for any job pauses
    until previously enqueued items are drained, and the dashboard sees the
    backpressure state transition to ``high`` and finally ``queue_full``.
    """
    ...
    async def pause_job(self, job_id: str) -> CrawlJob | None:
        ...
        # All workers are torn down; reset live counters so a later resume
        # has a clean slate even if a worker was cancelled mid-fetch.
        ctx.active_requests = 0
        ctx.job.stats.active_workers = 0
        await self._persist_state(ctx, force=True)
        self._update_global_stats()
        return ctx.job
        ...
        try:
            while True:
                ...
                try:
                    ...
                    # Decrement even on cancellation/exception so pause+resume
                    # can later reach the queue-empty completion gate.
    ...
    async def _respect_rate_limit(self, ctx: CrawlContext) -> None:
        min_interval = 1.0 / max(ctx.rate_limit_per_sec, 0.01)
        now = time.monotonic()
